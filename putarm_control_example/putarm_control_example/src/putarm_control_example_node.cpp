...
ros::Publisher ctrlPub2;//control the hand
std::vector<double> currentPos2 = {0,0,0,0};
...
void orderCallback2(const std_msgs::String::ConstPtr& msg){
    ...
    ctrlMsg2.goal.trajectory.points[0].positions.resize(4);


    if (msg->data.compare("close")==0)
    {
	ctrlMsg2.goal.trajectory.points[0].positions[0]=0.0;
	ctrlMsg2.goal.trajectory.points[0].positions[1]=1.4;
	ctrlMsg2.goal.trajectory.points[0].positions[2]=-1.4;
	ctrlMsg2.goal.trajectory.points[0].positions[3]=-1.4;
    }
    else if (msg->data.compare("open")==0)
    {
	ctrlMsg2.goal.trajectory.points[0].positions[0]=0.0;
	ctrlMsg2.goal.trajectory.points[0].positions[1]=0.0;
	ctrlMsg2.goal.trajectory.points[0].positions[2]=0.0;
	ctrlMsg2.goal.trajectory.points[0].positions[3]=0.0;
        ...
        ctrlMsg2.goal.trajectory.points[0].positions[0]=0.0;
    // Velocities
    ctrlMsg2.goal.trajectory.points[0].velocities.resize(4);
...
int main(int argc, char **argv) {
    //initialize node
    ros::init(argc, argv, "putarm_control_example");
    // node handler
    ros::NodeHandle n;
    // subsribe topic
